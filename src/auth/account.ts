...
// sessions, unused magic links, its queries (and the series + events
// extracted for them), and the calendar feed token. Idempotent — a retry
// after a partial failure simply finds the already-deleted rows gone.
export async function deleteAccount(db: Db, userId: string): Promise<void> {
  ...
  if (queryIds.length > 0) {
    const ids = queryIds.map(row => row._id);
    await db.collection('events').deleteMany({ query_id: { $in: ids } });
    await db.collection('series').deleteMany({ query_id: { $in: ids } });
  }
