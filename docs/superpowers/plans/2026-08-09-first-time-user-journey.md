...
- **Correction (2026-08-09, superseding an earlier "known gap" note in this plan):** every query tried while writing this plan came back with zero results, and that was wrongly attributed to a sandbox network restriction. The real cause: this instance's general-category engines (google, duckduckgo, brave, startpage, mojeek, qwant, bing) are configured as SearXNG "private engines" (container repo: `ansible/plays/templates/searxng/settings.yml.j2`, each with a `tokens:` list). Without a matching token, every engine is silently skipped and the JSON API returns `results: []` with no error and an empty `unresponsive_engines` — indistinguishable from "nothing found" unless you already know to look for this. **Required:** append `&tokens=<token>` to the search request. Token value lives in the container repo's `ansible/plays/vars/secrets.yml` (`vault_searxng_brave_token` — the google/duckduckgo/brave token vars there are all the same value), exposed to this app as `SEARXNG_TOKEN`. The same job-search app already depends on this (`job_search_searxng_token` in `ansible/plays/user.yml`).
- Rate limiting is not implemented by this plan (see `docs/design.md` §8, still open). Separately, production logs (`docker logs searxng.core` on `lehel.xyz`) show some of these engines (brave, qwant, startpage) do get CAPTCHA/rate-limit errors from their upstream providers even with a valid token — that's an existing, unrelated infra condition, not something this app's client needs to handle specially; a CAPTCHA'd engine just contributes no results for that query.
