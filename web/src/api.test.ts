...
describe('series api client', () => {
  it('listSeries fetches series for a query', async () => {
    ...
    const body = [{ id: 's1', title: 'Oktoberfest', appliesTo: 'Oktoberfest, Munich', description: 'd', searchKeywords: 'Oktoberfest Munich', sourceUrls: ['https://a.example'], status: 'candidate', eventCounts: { approved: 0, candidate: 0 }, previewEvents: [] }];
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => body }));
  ...
  it('reviewSeries posts approve and dismiss ids', async () => {
    ...
    const body = [{ id: 's1', title: 'Oktoberfest', appliesTo: 'Oktoberfest, Munich', description: 'd', searchKeywords: 'Oktoberfest Munich', sourceUrls: ['https://a.example'], status: 'approved', eventCounts: { approved: 0, candidate: 0 }, previewEvents: [] }];
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => body });
    vi.stubGlobal('fetch', fetchMock);
